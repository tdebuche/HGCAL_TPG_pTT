        ...
        for lay in range(len(Layers)):
            ...
            for module_idx in range(len(single_layer_modules)):
                ...
                if Layer <27:
                    ...
                    text_CEE +='\t\t Board_' + str(board) + ', Channel_' + str(channel)
                ...
                if Layer > 26 and ((lay!=len(Layers)-1 and module_type=='silicon') or (lay==len(Layers)-1 and module_type=='scintillator')):
                    if args.STCs == 'no':
                        channel =get_module_channel(Layer,module_type,module_u,module_v)
                        text_CEH +='\t\t Board_' + str(board) + ', Channel_' + str(channel)
                    ...
                    if args.STCs == 'yes':
                        ...
                        text_CEH +='\t\t Board_' + str(board) + ', Channel_' + str(channel) + ', Word_' + str(word) 
