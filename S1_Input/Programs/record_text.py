...
#record the numbering as S1_inputs (pTT part)
def record_input(args):
    ...
    with open('src/'+args.Modmap_version+'/STCs.json','r') as file:
        STCs = json.load(file)
    #create the numberings thanks to the "create_numbering" functions
    ...
    #create the text to record numberings
    text_CEE = 'Input CEE pTT Stage 1' + '\n\n'
    text_CEH = 'Input CEH pTT Stage 1'   '\n\n'
    #write the numbering board per board 
    for Board_number in range(14):
        #take the layer treated by this board
